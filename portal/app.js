...
async function init(){
  ...
  sb.auth.onAuthStateChange((_e,s)=>{ session=s; render(); if(s) load(); });
...
function bindLogin(){ document.getElementById('login')?.addEventListener('submit', async e=>{ e.preventDefault(); const fd=new FormData(e.currentTarget); const {error}=await sb.auth.signInWithPassword({email:fd.get('email'),password:fd.get('password')}); if(error) root.innerHTML=loginShell(`<div class="notice error">${escapeHtml(error.message)}</div>`), bindLogin(); }); }
